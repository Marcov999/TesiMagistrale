...
\begin{itemize}
    \item scriviamo $\Omega$ per indicare un \textit{dominio} in $\mathbb{C}^n$, vale a dire un aperto connesso;
    \item con \textit{varietà complessa} s'intende una varietà differenziabile reale, di dimensione pari e con i cambi di carta olomorfi se visti come fra aperti di $\mathbb{C}^n$;
    ...
    \item il disco unitario è $\mathbb{D}=\{z \in \mathbb{C} \mid |z|<1\}$, mentre $\mathbb{D}^n=\mathbb{D}\times\dots\times\mathbb{D}$ è il polidisco in $\mathbb{C}^n$ e $\mathbb{D}_r=\{z \in \mathbb{C} \mid |z|<r\}$ è il disco di raggio $r>0$;
...
Oltre ad avere curvatura negativa costante, la metrica e la distanza di Poincaré sono tali che le funzioni olomorfe dal disco unitario in sé sono semicontrazioni rispetto ad esse. Ciò discende dal seguente risultato.
...
Per la dimsotrazione si rimanda a \cite[Chapter I, paragraph 1, Theorem 1.1]{Ko2}. Ci tornerà utile anche la versione semplificata.

\begin{lm}
    (lemma di Schwarz) Sia $f\in\textnormal{Hol}(\mathbb{D},\mathbb{D})$ tale che $f(0)=0$. Allora per ogni $z\in\mathbb{D}$ si ha $|f(z)| \le |z|$ e $|f'(0)| \le 1$.
\end{lm}
...
\begin{oss} \label{metr_noncr}
    Non possiamo parlare di metrica perché, ad esempio, $K_{\mathbb{C}^n}\equiv 0$. Notiamo anche, data $f \in \text{Hol}(X,Y)$, che dalla definizione segue $K_Y\big(f(x);Df(x)Z\big) \le K_X(x;Z)$ per ogni $x \in X$ e $Z \in T_xX$. Ne consegue, restringendosi a una coordinata opportuna e applicando il lemma di Schwarz, che per le varietà $X$ complesse, embeddate in $\mathbb{C}^d$ e limitate $K_X$ è effettivamente una metrica; vale a dire, $K_X(z;Z)>0$ per ogni $z\in X$ e $0\not=Z\in T_zX$ (nella Proposizione \ref{metrica_bilip}, punto (1), dimostreremo qualcosa di più forte).
\end{oss}
...
\begin{defn}
    ...
    \begin{equation}\begin{split}
        k_X(z,w)=&\inf\Bigg\{\sum_{j=1}^m \omega(\zeta_{j-1},\zeta_j) \mid m \in \mathbb{N}, \zeta_j \in \mathbb{D}\text{ per }j=0,\dots,m\text{ tali che}\\
        &\text{esistono }\varphi_j\in \text{Hol}(\mathbb{D},X) \text{ con } \varphi_1(\zeta_0)=z,\varphi_m(\zeta_m)=w\text{ e}\\
        &\varphi_j(\zeta_j)=\varphi_{j+1}(\zeta_j)\text{ per }j=1,\dots,m-1\Bigg\}
    \end{split}\end{equation}
    per $z,w \in X$.
\end{defn}

\begin{oss} \label{k_lip}
    È facile vedere che $k_X$ è una pseudodistanza, ma in generale non è una distanza, ad esempio perché, come prima, $k_{\mathbb{C}^n}\equiv 0$; vedremo però più avanti che per le varietà $X$ complesse, connesse, embeddate in $\mathbb{C}^d$ e limitate $k_X$ è effettivamente una distanza. Inoltre, data $f \in \text{Hol}(X,Y)$, dalla definizione segue che $k_Y\big(f(x),f(y)\big) \le k_X(x,y)$ per ogni $x,y \in X$.
\end{oss}

\begin{defn}
    Sia $X$ una varietà complessa e connessa. Se $k_X$ è una distanza, diremo che $X$ è \textit{Kobayashi-iperbolica}.
\end{defn}

\begin{oss}
    Dalla definizione di $k_X$ segue che ogni varietà Kobayashi-iperbolica è uno \textit{spazio di lunghezze} nel senso di \cite[Part I, Paragraph 3, Definition 3.1]{BH}.
\end{oss}
