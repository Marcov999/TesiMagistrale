...
\begin{proof}
    ...
    Consideriamo una sottosuccessione di $\{F^\nu\}_{\nu\in\mathbb{N}}$ che converge uniformemente sui compatti a una funzione olomorfa $\tilde{F}:X \rightarrow Y$ con $\tilde{F}(X)\subseteq\overline{X}$. Poiché le iterate di $F$ sono compattamente divergenti, si deve avere che $\tilde{F}(X)\subseteq\partial_YX$. Allora, usando la Proposizione \ref{estraiunasotto-bis}, troviamo che $\tilde{F}$ è costante. Identifichiamo quindi
    ...
    Possiamo dunque scegliere, in modo simile a quanto fatto all'inizio della dimostrazione della Proposizione \ref{inf_impl_ugu} e applicando la Proposizione \ref{estraiunasotto-bis}, una sottosuccessione $\{\nu_j\}_{j\in\mathbb{N}}$ tale che:
    ...
    Consideriamo ora un $l\in\mathbb{N}$. Poiché la successione $\{F^{\mu_j-l}\}_{j\in\mathbb{N}}$ converge uniformemente sul compatto $\{F^l(o)\}$ a $\eta$, per il Lemma \ref{unaltrosottosotto-bis} converge, uniformemente su tutti i compatti di $X$, a $\eta$.
...
Vogliamo ora ottenere come conseguenza il teorema di tipo ``Wolff-Denjoy'' dimostrato in \cite{CMS}.

\begin{cor} \label{wd_type}
    (\cite[Theorem 1.15]{CMS}) Sia $X$ una sottovarietà complessa, connessa e limitata di $\mathbb{C}^n$. Supponiamo che $X$ sia taut e che esista un $\kappa_0>0$ tale che $X$ sia $(1,\kappa_0)$-visibile.
...
\begin{proof}
    Poiché $X$ è limitata, esiste $R>0$ tale che $X$ è una sottovarietà complessa e relativamente compatta della varietà Kobayashi-iperbolica $\mathbb{B}_R^n$, per cui anche $X$ è Kobayashi-iperbolica. Essendo anche taut e $(1,\kappa_0)$-visibile, soddisfa le ipotesi del Teorema \ref{wd_type_strong}, da cui la tesi.
\end{proof}
