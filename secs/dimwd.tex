...
\begin{thm} \label{periodico}
    (\cite[Theorem 0.4]{A3}) Sia $X$ una varietà taut e di tipo topologico finito. Supponiamo che $H^j(X;\mathbb{Q})=0$ per ogni $j>0$, e consideriamo una funzione $F\in\textnormal{Hol}(X,X)$. Allora la successione $\{F^h\}_{h\in\mathbb{N}}$ non è compattamente divergente se e solo se $F$ ha un punto periodico in $X$, cioè esistono $x\in X$ e $h_0\in\mathbb{N}$ tali che $F^{h_0}(x)=x$.
\end{thm}
